...
5. Stage a complete copy, run `scripts/repair_pagination_blank_tails.py`, and write the JSON report outside the staged tree.
...
Compare source and staged paths. In every changed DOCX, only `word/document.xml` may differ. Reopen the DOCX, confirm the named transition has no removable blank tail, and render every changed file; inspect each affected transition page for broken borders, missing headings, or clipping.
